...
export const ui = {
  ...
  ja: {
    nav: [
      ...
      { path: '/about', label: 'ツナグについて', short: '会社概要' },
    ],
    signup: '交流会に申し込む',
    signupShort: '申込',
    ...
    footerLinks: [
      ...
      { path: '/apply', label: 'プロフィール送付' },
      { path: '/about', label: 'ツナグについて' },
    ],
    footerTagline: '台湾の優秀な人財を、日本へ。',
    footerFree: '交流会への参加・就職マッチングは、台湾の皆さまに全て無料でご提供しています。',
    organizer: '主催：智囊和夥伴有限公司（Brain & Partner Taiwan Inc.）',
    card: {
      placements: (n: number) => `累計 ${n} 名が勤務`,
      fullTime: (n: number) => `${n} 名が正社員登用`,
      online: 'オンライン面接可',
      jlpt: (lv: string) => `${lv} 以上`,
    },
